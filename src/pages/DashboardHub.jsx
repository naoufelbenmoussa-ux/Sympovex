...
export default function DashboardHub() {
  ...
  const handlePasswordResetSubmit = (e) => {
    ...
    if (!success) {
      setPwError('Failed to change password. Please contact support.');
    }
  };

  if (!currentUser) {
    return (
      <div className="w-full min-h-[70vh] flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 text-center space-y-6 shadow-xl animate-in zoom-in-95 duration-200">
          ...
          <div className="space-y-2">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">Workspace Access Restricted</h3>
  ...
  if (currentUser?.requiresPasswordChange) {
    return (
      <div className="w-full min-h-[80vh] flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 space-y-6 shadow-2xl animate-in zoom-in-95 duration-200">
          <div className="text-center space-y-2">
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">Define New Password</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
              Your account was provisioned with a temporary password. For security, you must define a new password before you can access the dashboards.
            </p>
          </div>
          ...
          <form onSubmit={handlePasswordResetSubmit} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-2.5 bg-primary hover:bg-secondary text-white font-bold text-sm rounded-lg transition cursor-pointer shadow-md shadow-primary/10"
            >
              Update Password & Access Dashboards
            </button>
          </form>
        </div>
      </div>
    ...
    return (
      <div className="w-full min-h-[80vh] flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
        <div className="max-w-xl w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 space-y-6 shadow-2xl text-center animate-in zoom-in-95 duration-200">
          <div className="space-y-2">
            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Select Workspace Environment</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Welcome back, <strong className="text-slate-900 dark:text-white font-bold">{currentUser.name}</strong>. Your profile holds multiple role contexts. Please select your workspace environment:
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4 pt-4">
            {currentUser.roles.map(role => {
              ...
              if (role === 'Participant' || role === 'Author') {
                title = "Participant / Author Workspace";
                description = "Submit papers, check schedules, view E-posters, and message other delegates.";
              } else if (role === 'Reviewer') {
                title = "Academic Reviewer Panel";
                description = "Access assigned papers, evaluate submissions, and log review score cards.";
              } else if (role === 'PSC') {
                title = "PSC Program Chair Console";
                description = "Manage tracks, assign reviewers, validate papers, and oversee participants.";
              } else if (role === 'President') {
                title = "President / Administrator Console";
                description = "Configure details, brand colors, speakers list, and generate mail merge.";
              } else if (role === 'Superadmin') {
                title = "Superadmin Platform Hub";
                description = "Global SaaS administration: provision conference tenants and assign credentials.";
              }
              ...
              return (
                <button
                  ...
                  className="p-6 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 hover:border-primary hover:bg-primary/5 transition text-left space-y-2 cursor-pointer flex flex-col justify-between"
                >
                  <div>
      ...
      <div className="bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 py-3 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 text-xs font-semibold text-slate-500 font-mono">
          <div className="flex items-center gap-1.5">
            ...
            <span>SECURE WORKSPACE INTERFACE</span>
            <span className="text-slate-300">|</span>
            <span>TENANT: <span className="text-slate-900 dark:text-white font-bold">{currentConference?.name}</span></span>
          </div>

          <div className="flex items-center gap-1">
            <User className="w-3.5 h-3.5 text-primary" />
            <span>SESSION ROLE: <span className="text-primary font-bold">{currentUser?.role}</span></span>
          </div>
        </div>
      </div>
